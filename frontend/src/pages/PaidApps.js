...
import { ValueScale, Stack, Animation, EventTracker } from '@devexpress/dx-react-chart';
...
export default function PaidApps() {
  const [vsData, setVsData] = React.useState([]);
  const { data } = useSWR('/api/paidvsfreeandprice');
  if (data && vsData.length === 0) {
    const m2009 = new Map();
    const m2010 = new Map();
    const m2011 = new Map();
    const m2012 = new Map();
  ...
  return (
    <Paper>
      <Chart data={vsData}>
        ...
        <BarSeries name="Paid" valueField="Paid" argumentField="year" scaleName="Paid" />
        <LineSeries name="Price" valueField="Price" argumentField="year" scaleName="Price" />
